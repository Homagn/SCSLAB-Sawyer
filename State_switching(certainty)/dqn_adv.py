...
def writelog(reward,done,p1,p2,values1,values2,message):
    ...
    fields1=[np.sum(reward),done,p1,p2,values1,values2,message]
...
class DQNAgent:
    def __init__(self, env, action_size, switches):
        ...
        if(self.continuation==True):
            ...
            self.target_model_head.load_weights("target_model_weights_head.h5")
            self.target_model_hand.load_weights("target_model_weights_hand.h5")
            #print("Loading the past experience ")
            #file = open('Experience/experience_head1.obj', 'r')
            #self.memory_head = pickle.load(file)
            #file.close()
            #print("Loading the past experience ")
            #file = open('Experience/experience_hand1.obj', 'r')
    ...
    def act(self, state):
        ...
        avg_certainty=[] #This value would be calculated as avg. distance from max entropy

        for i in range(0,len(a_v),3):#3 possible actions for each joint: increase, decrease or remain same
            j=a_v[i:i+3]
            k=j-np.min(j)#remove negative values
            print("k+1 ",k+[1.0,1.0,1.0])
            #[1,1,1] is the max entropy case when the agent predicts equal q values for all the moves associated with that joint
            avg_certainty.append(entropy([1,1,1])-entropy(k+[1.0,1.0,1.0])) 

            acts.append(np.argmax(j)+i) #Major bug fix
        s_v=act_values[0][-self.switches:]

        avg_certain=np.mean(avg_certainty)
        ...
        if(self.evaluate==False):
            if ((np.random.rand() <= self.epsilon)):# or len(self.memory_head)<200 or len(self.memory_hand)<200):
                ...
                return avg_certain,np.concatenate((acts,switches)),acts,acts,"random"
        
        return avg_certain,acts,self.model_head.predict([state["image"],state["joints"]]),self.model_hand.predict([state["image"],state["joints"]]),self.view_state
    ...
    def replay(self, batch_size):
        ...
        if(self.view_state=="head"):
            ...
            model_switch_target=self.target_model_hand
        ...
        if(self.view_state=="hand"):
            ...
            model_switch_target=self.target_model_head
        ...
        for state, action, reward, next_state, done, switched in minibatch:
            ...
            if not done:
                model_pred=[]
                if(switched==True): #If switched is true, regular Q learning update style is reversed
                    model_pred=model_switch_target.predict([next_state["image"],next_state["joints"]])[0] #Ensures that one type of model sees one type of image only
                if(switched==False):
                    model_pred=model_target.predict([next_state["image"],next_state["joints"]])[0] #regular Q learning formula
                target1 = reward[0] + self.gamma * np.amax(model_pred[0:3])
                target2 = reward[1] + self.gamma * np.amax(model_pred[3:6])
                target3 = reward[2] + self.gamma * np.amax(model_pred[6:9])
                target4 = reward[3] + self.gamma * np.amax(model_pred[9:12])
                target5 = reward[4] + self.gamma * np.amax(model_pred[12:15])
                target6 = reward[5] + self.gamma * np.amax(model_pred[15:18])
                target7 = reward[6] + self.gamma * np.amax(model_pred[18:21])
                target8 = reward[7] + self.gamma * np.amax(model_pred[21:23])
            target_f =[]
            if(switched==True):
                target_f = model.predict([state["image"],state["joints"]]) #Ensures that one type of model sees one type of image only
            if(switched==False):
                target_f = model.predict([state["image"],state["joints"]]) #regular Q learning formula
...
if __name__ == "__main__":
    ...
    for e in range(episodes):
        ...
        state = env.reset(agent.evaluate)
        for time_t in range(30): #Gives 50 tries to the robot to keep moving the arm towards the goal
            state["image"]=np.reshape(state["image"], [1, 1,agent.imsize,agent.imsize])
            a_c,action,values1,values2,message = agent.act(state) #batch_size=1, num_channels=1
            ...
            if(switch==0):
                ...
                #env is given information about avg certainty and in return it puts that information in the reward structure
                next_state, reward, done, performance1, performance2 = env.step(actions,a_c)
        ...
        if(agent.evaluate==False):
            if(agent.cur_mem_head-agent.prev_mem_head>32):
                ...
                agent.view_state="head"
                agent.replay(32) #Use 32 in simulation
