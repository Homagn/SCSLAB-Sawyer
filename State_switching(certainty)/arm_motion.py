...
class arm(object):
    ...
    def orient_handcam(self):
        ...
        self.limb.move_to_joint_positions(wave,timeout=1.5,threshold=0.005)
        rospy.sleep(0.2)
    ...
    def robot_locked_away(self):
        ret=False
        end_cartesian=copy.copy(self.limb.endpoint_pose())
        cart=end_cartesian.values()
        if(cart[0].z<0.009 or cart[0].z>0.52 or cart[0].x<0.27 or cart[0].y>0.44 or cart[0].y<-0.43 or cart[0].x>0.81):
            return True
    ...
    def move_arm(self,actions):
        ...
        wave = {'right_j6': self.angles['right_j6'], 'right_j5': self.angles['right_j5'], 'right_j4': self.angles['right_j4'], 'right_j3': self.angles['right_j3'], 'right_j2': self.angles['right_j2'], 'right_j1': self.angles['right_j1'], 'right_j0': self.angles['right_j0']}
        self.limb.move_to_joint_positions(wave,timeout=1.0,threshold=0.005)
        rospy.sleep(0.2)
'''
def main():
    am=arm()
